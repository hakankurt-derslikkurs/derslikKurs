...
// Simülasyon sınavı başvuru tipi
interface SimulasyonSinaviBasvuru {
  // Öğrenci Bilgileri
  name: string
  surname: string
  tc: string
  ...
  try {
    ...
    try {
      const rateLimitResult = await supabase.rpc('check_rate_limit', {
        ...
        p_endpoint: 'submit-simulasyon-sinavi-basvuru',
        p_window_minutes: 5,
        p_max_requests: 20
      })
    ...
    let requestData: SimulasyonSinaviBasvuru
    try {
    ...
    if (!['9', '10'].includes(requestData.grade)) {
      await logValidationError("Geçerli bir sınıf seçiniz (9, 10)")
      return new Response(JSON.stringify({
        success: false,
        error: "Geçerli bir sınıf seçiniz (9, 10)"
      }), {
        status: 400,
        headers: corsHeaders
      })
    ...
    if (!requestData.province || requestData.province.trim().length < 2) {
      await logValidationError("İl seçiniz")
      return new Response(JSON.stringify({
        success: false,
        error: "İl seçiniz"
      }), {
    ...
    if (!['online', 'yuzYuze'].includes(requestData.examType)) {
    ...
    // TC ile daha önce başvuru yapılmış mı kontrol et
    const { data: existingApplications, error: checkError } = await supabase
      .from('simulasyon_sinavi_basvuru')
      .select('tc_kimlik_no')
    ...
    return new Response(JSON.stringify({
      success: true,
      message: "Simülasyon sınavı başvurunuz başarıyla alındı. T.C. Kimlik No: " + requestData.tc,
